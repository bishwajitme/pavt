...
class SelectBox extends Component {
  ...
  render () {
    ...
    if (status === 'done' && response) {
      ...
      const current = filter.getIn(
        ['education', 'temp', type.singular, 'title']
      );
      const options = response.get(type.plural).push(Immutable.fromJS({
        slug: '',
        title: `Alla ${type.label}`,
        school: '',
      }));
      ...
      const getDesktopOptions = () => {
        return options.sortBy(item => item.get('slug')).map(item => {
          ...
          const active = (title === current) ? 'active' : '';
          return (
            <div key={value}
                 data-value={value}
                 data-title={title}
